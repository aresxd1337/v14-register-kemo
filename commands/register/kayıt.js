...
${emoji.kirmiziok} üyenin önceki isimlerine \`${config.bot.prefix}isimler <@kemo/ID>\` komutuyla bakarak kayıt İşlemini gerçekliştirmeniz önelir.`)
        ...
        collector.on("collect", async (button) => {
          ...
          if(button.customId === "erkek") {
            ...
            const kemo = new MessageEmbed().setColor('RANDOM').setAuthor({name: message.member.displayName, iconURL: message.guild.iconURL({ dynamic: true})}).setFooter({text:(`Toplam kayıt: ${db.get(`toplam_${message.author.id}`) || 0} (${db.get(`kadın_${message.author.id}`) || 0} Kadın, ${db.get(`erkek_${message.author.id}`) || 0} Erkek)`) , iconURL: message.guild.iconURL({ dynamic: true})})
            if (!names) {
              kemo.setDescription(`${emoji.nbdy_man} Kullanıcının ismi \`${name} ${config.register.symbol} ${age}\` olarak değiştirildi ve <@&${config.register.ekrekRolü}> rolü verilerek kayıt edildi.`) 
            } else {
              kemo.setDescription(`${emoji.nbdy_man} Kullanıcının ismi \`${name} ${config.register.symbol} ${age}\` olarak değiştirildi ve <@&${config.register.ekrekRolü}> rolü verilerek kayıt edildi. \n\n ${emoji.kirmiziok} Kullanıcının toplamda **${names.length}** isim kayıtı görüntülendi. isimleri görüntülemek için \`${config.bot.prefix}isimler [kemo/ID]\` komundunu kullanabilirsiniz.`) 
            }
    ...
    msg.edit({
      ...
      if(button.customId === "kadin") {
        ...
        const guild = message.guild
            ...
            await guild.members.cache.get(member.id).roles.remove(config.register.kayitsiz)
        const kemo = new MessageEmbed().setColor('RANDOM').setAuthor({name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true })}).setFooter({text: (`Toplam kayıt: ${db.get(`toplam_${message.author.id}`) || 0} (${db.get(`kadın_${message.author.id}`) || 0} Kadın, ${db.get(`erkek_${message.author.id}`) || 0} Erkek)`) , iconURL: message.guild.iconURL({ dynamic: true})})
        if (!names) {
            kemo.setDescription(`${emoji.nbdy_girl} Kullanıcının ismi \`${name} ${config.register.symbol} ${age}\` olarak değiştirildi ve <@&${config.register.KadinRolü}> rolü verilerek kayıt edildi.`) 
        } else {
            kemo.setDescription(`${emoji.nbdy_girl} Kullanıcının ismi \`${name} ${config.register.symbol} ${age}\`olarak değiştirildi ve <@&${config.register.KadinRolü}> olarak kayıt edildi. \n\n ${emoji.kirmiziok} Kullanıcının toplamda **${names.length}** isim kayıtı görüntülendi. isimleri görüntülemek için \`${config.bot.prefix}isimler [kemo/ID]\` komundunu kullanabilirsiniz`) 
        }
